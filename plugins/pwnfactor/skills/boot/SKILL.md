...
- **CLAUDE.md gate contract** → append (never overwrite): *"Any code-touching change runs `/pwnfactor:gg` before integrate and follows `/pwnfactor:swarm` isolation + rails. Data-/fleet-mutating work ships with dry-run + checkpoint + rollback. Before a production push or big build, OFFER `/pwnfactor:sweep` - the operator decides. `/pwnfactor:gg` writes `.pwnfactor/gate.json`, which pwnfactor's stop-hook verifies. **That gate closes at COMMIT time.** While a build is in flight the hook fires on every stop - that is the normal mid-build state, so acknowledge it in one line and keep going; `/pwnfactor:gg` runs ONCE, on the assembled diff, when the change is ready to commit. **Never end a turn with the plan non-empty and no work in flight.**"*
