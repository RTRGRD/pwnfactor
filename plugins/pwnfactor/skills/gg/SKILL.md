...
| **SOLO** | ROUTINE, small (roughly <=50 changed lines), one surface, zero HIGH signals | **Codex (regular) PLUS one `panel-code-review` subagent on the tier's model** - two independent looks, one of them cross-model. Codex carries the cross-family catch and costs zero Claude tokens; the single Claude reviewer exists because a lone reviewer has nothing to disagree with, and this skill's own rule is that a reviewer is a lead, not a fact. No simplifier, no security agent (their surfaces are what SOLO already excluded). If Codex is unavailable (health-check verified, never assumed), the Claude reviewer runs alone and the report says so |
...
At SKIP there is nothing to spawn - write the gate and stop. At SOLO spawn `panel-code-review`
alone alongside Codex (step 4). At PANEL/ADVERSARIAL spawn all three:
...
### 4. Codex cross-model review - per depth, if available
**Codex runs at every depth except SKIP** - SOLO and PANEL regular, ADVERSARIAL adversarial. It is
...
Claude reviewers structurally share. Codex findings still get step 5's scrutiny at every depth.
...
- **Cost:** the tier controls the model; the DEPTH controls how many reviewers run. Docs-only is SKIP (0 reviewers), a small routine diff is SOLO (Codex + 1), a real feature is PANEL (3 + Codex), risk is ADVERSARIAL (3 on Opus + adversarial Codex). Depth never argues down past a HIGH signal, and Codex runs at every depth that reviews anything.
