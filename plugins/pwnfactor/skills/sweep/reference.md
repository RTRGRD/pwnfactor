...
├── .claude/
│   ├── commands/security-audit.md     ← the /security-audit slash command (the skill)
│   └── agents/security-auditor.md     ← companion "security-auditor" sub-agent
├── bin/
...
## Install into a project (for a Claude Code agent or a human)

From the target repo's root, copy these three things in (keep the relative layout):

1. `.claude/commands/security-audit.md`  → `<repo>/.claude/commands/security-audit.md`
2. `.claude/agents/security-auditor.md`  → `<repo>/.claude/agents/security-auditor.md`
3. `bin/run-security-audit.ps1`          → `<repo>/bin/run-security-audit.ps1`

The slash command invokes the script via the relative path `bin/run-security-audit.ps1`. If you keep the
script somewhere else, update the `How to run` paths in `security-audit.md` to match.

Then add to the target repo's `.gitignore`:
...
Restart Claude Code in the target repo and run `/security-audit`, or run the script directly (below).
